        ...
        <div class="card">
            ...
            <div class="body">
                ...
                @endif
                    ...
                    @else
                        {!! Form::open(array('url' => "dopes",'method' => 'POST', 'enctype' =>'multipart/form-data')) !!}
                    @endif
...
{{--            {{Form::label("id", "ID")}}--}}
{{--            {{Form::text('id',--}}
{{--                old('id') ? old('id') : (!empty($dope) ? $dope->id : null),--}}
...
{{--                ["class" => 'form-control',"id" => "thana","placeholder" => "Enter your Police Station"]) }}--}}
{{--        </div> <!-- end form-group -->--}}
        </div>
        ...
        @section('script2')
            ...
            <script>
                ...
                function addRow(){
                    i++;
                    var MR_Row = "";
                    MR_Row += "<tr class='purchaseItem' id='Purchase'>";
                    {{--MR_Row += '<td>{{Form::select("test_name[]", $tests,null,["class" => "form-control","placeholder" => "Select Category", ])}}</td>';--}}
                    MR_Row += '<td>{{Form::select("received_type[]", ['cash'=>'Cash', 'cheque'=>'Cheque', ],null,["class" => "form-control","placeholder" => "Select Category", ])}}</td>';
                    MR_Row += '<td ><input style="text-align: right" type="text" name="amount[]" class="form-control" placeholder="0.00"></td>';
                    MR_Row += '<td><button class="btn btn-danger btn-sm deleteItem" id="" type="button"><i class="fa fa-minus"></i></button></td>';
                    MR_Row += "</tr>";
